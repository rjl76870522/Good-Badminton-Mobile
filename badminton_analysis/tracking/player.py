...
class PlayerTracker:
    ...
    def update(self, frame_index, centroids, ball_image_position, left_hand_positions, right_hand_positions, detect_frame_count):
        ...
        # Use court coordinates (meters) instead of image pixels for half assignment
        # Net is at Y=6.7m on a standard 13.4m court
        COURT_NET_Y = 6.7
        upper_court_centroids = []
        lower_court_centroids = []
        for centroid in centroids:
            cp = self.court_mapper.image_to_court(centroid) if self.court_mapper else None
            if cp is not None and len(cp) > 1:
                court_y = float(cp[1])
            else:
                # Fallback to image-space threshold
                court_y = 0.0 if centroid[1] < self.threshold else 13.4
            if court_y < COURT_NET_Y:
                upper_court_centroids.append(centroid)
            else:
                lower_court_centroids.append(centroid)

        if len(upper_court_centroids) > 1:
            # Pick the one closest to court center, not just the topmost
            court_center_x = 3.05
            upper_court_centroids.sort(key=lambda p: abs(
                self.court_mapper.image_to_court(p)[0] - court_center_x
                if self.court_mapper.image_to_court(p) is not None and len(self.court_mapper.image_to_court(p)) > 0
                else 999
            ))
            upper_court_centroids = [upper_court_centroids[0]]
        if len(lower_court_centroids) > 1:
            court_center_x = 3.05
            lower_court_centroids.sort(key=lambda p: abs(
                self.court_mapper.image_to_court(p)[0] - court_center_x
                if self.court_mapper.image_to_court(p) is not None and len(self.court_mapper.image_to_court(p)) > 0
                else 999
            ))
            lower_court_centroids = [lower_court_centroids[0]]
        ...
        for centroid in filtered_centroids:
            try:
                ...
                self._update_player_position(
                    region, centroid, left_hand, right_hand, players_record, frame_index
                )
    ...
    def _update_player_position(
        self, region, centroid, left_hand_pos, right_hand_pos, players_record, frame_index
    ):
