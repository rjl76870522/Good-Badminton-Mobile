    ...
    LegalDocumentType.privacy => (
        ...
        sections: [
          ...
          (
            title: '四、存储与保护',
            body:
                '分析任务、原始视频、报告、图表和分析视频保存在项目自管的中心服务器，直至用户在训练历史中主动删除对应记录。本机离线报告由用户自行管理。我们采用 HTTPS、Cloudflare Tunnel、访问隔离和数据库备份等措施保护数据，但互联网传输无法保证绝对安全。',
          ),
          (
            title: '五、信息共享',
            body:
          ...
          (
            title: '六、用户权利与未成年人保护',
            body:
                '用户可以在系统设置中撤回相机、相册、定位和通知权限，也可以清除离线报告或删除训练记录。如需查询、更正或删除数据，可通过支持页面或 jialeR01@126.com 联系项目团队。未成年人应在监护人指导下使用本服务，不应上传包含他人敏感信息的视频。',
          ),
          (
            title: '七、联系我们',
            body:
                '运营主体为智羽项目团队。隐私问题、数据删除、投诉建议和安全问题可通过 https://www.audacity6441.kdns.fr/support 或 jialeR01@126.com 联系。我们会在收到可识别请求后尽快处理。',
          ),
        ],
      ),
    LegalDocumentType.agreement => (
        ...
        sections: [
          ...
          (
            title: '五、协议更新与联系',
            body:
                '功能或法规发生变化时，本协议可能更新。重要变化会通过应用或网站说明。继续使用更新后的服务视为接受新版本；如有异议，可停止使用并通过 jialeR01@126.com 申请删除数据。',
          ),
        ],
      ),
    LegalDocumentType.personalInformation => (
        ...
        sections: [
          (
            title: '匿名用户标识',
            body:
                '场景：区分不同用户的任务与历史记录。方式：优先使用设备侧生成或派生的稳定匿名标识，无法获取时随机生成。范围：不要求真实姓名、手机号或身份证。保存位置：本机及中心服务器。',
          ),
          (
            title: '训练视频与球场角点',
            body:
                '场景：生成轨迹、热力图、精彩片段和训练报告。方式：用户主动选择或从合作球馆主动获取。保存位置：中心服务器，直至用户删除训练记录；本机是否另行保存由用户决定。',
          ),
          (
            title: '任务与报告信息',
            body:
    ...
    LegalDocumentType.cooperation => (
        ...
        sections: [
          ...
          (
            title: '联系渠道',
            body:
                '请通过智羽支持页面或 jialeR01@126.com 说明合作单位、联系人、合作场景和预计规模，项目团队确认后再沟通技术与运营方案。',
          ),
        ],
      ),
  };
